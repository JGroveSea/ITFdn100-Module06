...
# Jgrove,11.15.19,Modified code to complete assignment 6
# ------------------------------------------------------------------------ #
...
class FileProcessor:
    ...
    def WriteListDataToFile(file_name, list_of_rows):
        ##First pieces written for assignment
        strFileName.write(lsttable)
        strFileName.close()
    # TODO: Create more functions that perform various Processing task as needed

    @staticmethod
    def Add_Item_To_List(strTask,strPriority,lstTable):
    ...
    @staticmethod
    def Remove_Item_From_List(strTask):
        ...
        while (intRowNumber < len(lstTable)):
            if (strTask == str(list(dict(lstTable[intRowNumber]).values())[0])):  # Search current row column 0
    ...
    @staticmethod
    def Save_File(strFileName,lstTable):
...
while(True):
    ...
    # Step 3.2 - Add a new item to the list/Table
        ...
        strTask,strPriority = IO.new_task()
        ...
        lstTable = FileProcessor.Add_Item_To_List(strTask,strPriority,lstTable)
    ...
    # Step 3.3 - Remove a new item to the list/Table
        ...
        FileProcessor.Remove_Item_From_List(strKeyToRemove)
        IO.ShowCurrentItemsInList(lstTable)  # Show current data in the list/table
        continue  # to show the menu
    ...
    # Step 3.4 - Save tasks to the ToDoFile.txt file
        ...
        if("y" == str(input("Save this data to file? (y/n) - ")).strip().lower()):  # Double-check with user
            FileProcessor.Save_File(strFileName,lstTable)
